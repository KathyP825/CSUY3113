...
#define LoseScreen_WIDTH 14
#define LoseScreen_HEIGHT 8

unsigned int losescreen_data[] = {
...
void LoseScreen::Initialize() {
    ...
    state.map = new Map(LoseScreen_WIDTH, LoseScreen_HEIGHT, losescreen_data, mapTextureID, 1.0f, 16, 10);
...
void LoseScreen::Render(ShaderProgram* program) {
    ...
    Util::DrawText(program, fontTextureID, "You Lose!", 0.4f, 0.1f, glm::vec3(3.0f, -3.0f, 0.0f));
    Util::DrawText(program, fontTextureID, "Alien failed to escape...", 0.2f, 0.01f, glm::vec3(2.5f, -4.0f, 0.0f));
}